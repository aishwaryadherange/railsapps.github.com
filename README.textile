...
* On its own, the GitHub wiki does not provide everything needed for the project website. I wanted to improve the layout and design to make the site more attractive and useful. I wanted to add Javascript for the Disqus commenting system as well as a Google Analytics tracking script. I would use a tool such as "Jekyll":https://github.com/mojombo/jekyll to combine the wiki files with a layout template and generate static web pages to upload to GitHub for site hosting.
...
When you are ready to generate the static pages, use the gollum-site gem to generate the static web pages in the wiki repository. Gollum-site has a built-in web server if you want to view the site locally before committing and publishing to GitHub.
